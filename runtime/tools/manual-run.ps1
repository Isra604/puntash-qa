...
$statusPath = Join-Path $stateDir 'MANUAL_SCAN_STATUS.json'
...
$logDir = Join-Path $stateDir 'manual\logs'
...
if ($policy.schedule.executor_mode -eq 'AGENT_MANAGED') {
    Save-Status 'NEEDS_AGENT' 'This project uses an external AI/platform runner. SCAN NOW cannot start that external runner from this local Dashboard.'
    exit 7
}
if ($policy.schedule.executor_mode -ne 'LOCAL_COMMAND') {
    Save-Status 'NEEDS_SETUP' 'A scan runner has not been configured yet. Choose how PUNTASH QA should run scans in Schedule & setup.'
    exit 7
}
...
try {
    ...
    $promptPath = Join-Path $installRoot 'prompts\MANUAL_QA.md'
if(-not(Test-Path $promptPath)){$promptPath=Join-Path $installRoot 'templates\MANUAL_QA.md'}
    $arguments = @()
    ...
    Save-Status 'RUNNING' 'PUNTASH QA scan started.' @{ run_id = $runId; executor = $command; started_at = (Get-Date).ToString('o'); acceptance_receipt_integrity = $(if($legacyAcceptanceReceipt){'LEGACY_UPDATE_RECEIPT'}else{'HASH_VERIFIED'}) }
    ...
    if ($exitCode -eq 0) {
        Save-Status 'SUCCESS' 'PUNTASH QA scan completed.' @{ run_id = $runId; exit_code = $exitCode; stdout = $stdoutPath; stderr = $stderrPath; completed_at = (Get-Date).ToString('o') }
        exit 0
    }
    ...
    Save-Status 'FAILED' ('Manual scan runner internal error: ' + $_.Exception.Message) @{ completed_at = (Get-Date).ToString('o') }
    exit 10
} finally {
