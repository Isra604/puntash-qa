...
def release_readiness(runs):
    if not runs:
        return {'state': 'NOT_SCANNED', 'label': 'Could not verify', 'ready': False, 'reasons': ['Run a full scan before deciding whether this project is ready to release.']}
    r = runs[0]
    s = r.get('summary') or {}
    fail = int(s.get('fail', 0) or 0)
    blocked = int(s.get('blocked', 0) or 0)
    not_run = int(s.get('not_run', 0) or 0)
    if fail or blocked:
        reasons = []
        ...
        return {'state': 'NOT_READY', 'label': 'Not yet', 'ready': False, 'reasons': reasons}
    if not_run:
        return {'state': 'INCOMPLETE', 'label': 'Could not fully verify', 'ready': False, 'reasons': [f'{not_run} quality check(s) were not run.']}
    if len(r.get('gates') or []) != 25 or len(r.get('lenses') or []) != 9:
        return {'state': 'INCOMPLETE', 'label': 'Could not fully verify', 'ready': False, 'reasons': ['The latest scan does not contain the complete 25-check and 9-lens record.']}
    path = latest_run_file(runs)
    if not path:
        return {'state': 'INCOMPLETE', 'label': 'Could not fully verify', 'ready': False, 'reasons': ['The latest structured scan record is unavailable.']}
    ...
    try:
        ...
        if rr.returncode != 0:
            return {'state': 'INCOMPLETE', 'label': 'Could not fully verify', 'ready': False, 'reasons': ['The latest scan record did not pass PUNTASH QA evidence validation.']}
    except Exception:
        return {'state': 'INCOMPLETE', 'label': 'Could not fully verify', 'ready': False, 'reasons': ['The latest scan record could not be validated.']}
...
def diagnostics(project: Path):
    ...
    if sched_state in {'POLICY_INVALID', 'REGISTRATION_STATE_INVALID', 'SCHEDULER_STATUS_ERROR', 'STALE_LOCAL_REGISTRATION', 'MISSING_LOCAL_REGISTRATION', 'BLOCKED'}:
        ...
        issues.append({'code': sched_state, 'severity': 'INFO', 'title': 'Your AI platform still has one scheduling step', 'message': str(sched_reg.get('message') or ''), 'action': None})
    return {
...
def activity(project: Path, runs):
    ...
    for h in read_jsonl(STATE / 'OWNER_POLICY_HISTORY.jsonl', 80):
        events.append({'at': str(h.get('changed_at') or ''), 'type': 'POLICY', 'title': 'Permissions or schedule changed', 'detail': f"Mode: {h.get('preset','unknown')} · automatic scans: {'on' if h.get('schedule_enabled') else 'off'}", 'policy_revision': h.get('revision')})
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def api_get(self, path, query):
            ...
            if path == '/api/overview':
                ...
                return self.send_json({'ok': True, 'overview': {'data': data, 'policy': pol, 'scheduler': sched, 'release_readiness': release_readiness(runs), 'diagnostics': diagnostics(project)}})
            ...
            if path == '/api/scan-status':
                ...
                proc = scan_proc[0]
                active = bool(proc is not None and proc.poll() is None) or str(status.get('last_result')) == 'RUNNING'
        ...
        def do_POST(self):
            ...
            if path == '/api/approval':
                ...
                if not REQUEST_ID.fullmatch(rid) or decision not in {'APPROVE', 'DENY'}: return self.send_json({'ok': False, 'error': 'invalid_approval_request'}, 400)
                with approval_lock:
                    ...
                    record = {'request_id': rid, 'request_hash': request_hash(req), 'decided_at': now(), 'decision': decision, 'finding_id': req.get('finding_id'), 'change_summary': str(req.get('change_summary') or '')[:1000], 'policy_revision': req.get('policy_revision')}
                    if decision == 'DENY':
            ...
            if path == '/api/recovery':
                ...
                try:
                    ...
                    subprocess.run(scheduler_tool + ['remove', '--owner-approved'], capture_output=True, text=True, shell=False)
                    saved = run_json(policy_tool + ['get'])
                    collect_dashboard_data(project)
                    return self.send_json({'ok': True, 'message': 'Permission settings were reset to Observe only. Automatic scans are off.', 'policy': saved})
                except Exception as exc:
