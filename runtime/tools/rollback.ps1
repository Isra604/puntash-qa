...
foreach($d in @('gates','templates')){$target=Join-Path $installRoot $d;if(Test-Path $target){Remove-Item $target -Recurse -Force};Copy-Item (Join-Path $BackupPath $d) $target -Recurse -Force}
...
foreach($f in @('AGENT_INSTRUCTIONS.md','LICENSE','NOTICE','CREDITS.md','TERMS_OF_USE.md','DISCLAIMER.md','DATA_RESPONSIBILITY_NOTICE.md','HUMAN_ACCEPTANCE.md','TERMS_VERSION','LEGAL_MANIFEST.json','INSTALLATION.json')){$src=Join-Path $BackupPath $f;if(Test-Path $src){Copy-Item $src (Join-Path $installRoot $f) -Force}}
