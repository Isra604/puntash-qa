...
try {
  ...
  if($newVersion -ge [version]'2.2.0'){
    foreach($required22 in @('runtime\templates\MANUAL_QA.md','runtime\prompts\MANUAL_QA.md','runtime\tools\manual-run.ps1','runtime\tools\manual-run.py','runtime\tools\manual-run.sh','runtime\tools\project-fingerprint.ps1','runtime\tools\project-fingerprint.py','runtime\tools\project-fingerprint.sh','runtime\dashboard\index.html')){if(-not(Test-Path (Join-Path $pkgRoot $required22))){throw "Downloaded v2.2 package missing: $required22"}}
  }
  ...
  if($newVersion -ge [version]'2.2.0'){foreach($r22 in @('templates\MANUAL_QA.md','prompts\MANUAL_QA.md','tools\manual-run.ps1','tools\manual-run.py','tools\manual-run.sh','tools\project-fingerprint.ps1','tools\project-fingerprint.py','tools\project-fingerprint.sh','dashboard\index.html')){if(-not(Test-Path (Join-Path $installRoot $r22))){throw "Post-update validation failed: missing v2.2 dashboard asset: $r22"}}}
