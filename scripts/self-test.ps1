...
$runTemplate=Get-Content (Join-Path $root 'runtime\templates\DASHBOARD_RUN.json') -Raw|ConvertFrom-Json;Check ([int]$runTemplate.schema_version -ge 3 -and $null-ne$runTemplate.automatic_remediation) 'schema-v3+ remediation accounting compatibility contract'
  $dash=Get-Content (Join-Path $root 'runtime\dashboard\index.html') -Raw;if([version]$version -lt [version]'2.2.0'){Check ($dash.Contains('Agent permissions') -and $dash.Contains('Scheduled QA') -and $dash.Contains('QA Control Center')) 'dashboard v2.1 control UI present';Check ($dash.Contains('openAuthenticatedReport') -and $dash.Contains('X-QA-Control-Token')) 'dashboard authenticated report control present'}else{Check ($dash.Contains('SCAN NOW') -and $dash.Contains('Things to review') -and $dash.Contains('Automatic scans') -and $dash.Contains('Fix PUNTASH QA') -and $dash.Contains('Ask PUNTASH')) 'dashboard v2.2 human control UI present';Check ($dash.Contains('/api/evidence') -and $dash.Contains('/api/approval') -and $dash.Contains('X-QA-Control-Token')) 'dashboard v2.2 authenticated control surface present'}
}
if([version]$version -ge [version]'2.2.0'){
  $v22=@('runtime\templates\MANUAL_QA.md','runtime\prompts\MANUAL_QA.md','runtime\tools\manual-run.ps1','runtime\tools\manual-run.py','runtime\tools\manual-run.sh','runtime\tools\project-fingerprint.ps1','runtime\tools\project-fingerprint.py','runtime\tools\project-fingerprint.sh','scripts\v2.2-dashboard-red-team.py','scripts\v2.2-windows-dashboard-red-team.ps1','scripts\v2.2-final-adversarial-red-team.py','scripts\v2.2-windows-final-adversarial-red-team.ps1','scripts\v2.2-dashboard-usability-red-team.py','scripts\v2.2-dashboard-browser-smoke.py','scripts\v2.2-upgrade-red-team.ps1','docs\V2_2_DASHBOARD_CAPABILITY_MATRIX.md','docs\V2_2_DASHBOARD_CHECKPOINT.json','docs\PUNTASH_QA_DASHBOARD_RELEASE_MASTER_PLAN.md')
  ...
  $brain=Get-Content (Join-Path $root 'runtime\AGENT_INSTRUCTIONS.md') -Raw;Check ($brain.Contains('final project state that was actually verified') -and $brain.Contains('immediately before final run validation/report closure')) 'v2.2 final-state fingerprint brain contract'
  $schedA=(Get-FileHash (Join-Path $root 'runtime\prompts\SCHEDULED_QA.md') -Algorithm SHA256).Hash;$schedB=(Get-FileHash (Join-Path $root 'runtime\templates\SCHEDULED_QA.md') -Algorithm SHA256).Hash;Check ($schedA-eq$schedB) 'v2.2 scheduled prompt compatibility copy matches canonical prompt'
  $v22Template=Get-Content (Join-Path $root 'runtime\templates\DASHBOARD_RUN.json') -Raw|ConvertFrom-Json;Check ([int]$v22Template.schema_version-eq4 -and [string]$v22Template.project.fingerprint.algorithm-eq'PUNTASH_SOURCE_V1') 'v2.2 schema-v4 project fingerprint contract'
  $agentV22=Get-Content (Join-Path $root 'runtime\AGENT_INSTRUCTIONS.md') -Raw;Check ($agentV22.Contains('New v2.2 runs use schema 4') -and $agentV22.Contains('tools/project-fingerprint')) 'v2.2 agent requires project freshness snapshot'
}
...
try{
 ...
 & pwsh -NoProfile -File $validator -RunPath $valid|Out-Null;Check ($LASTEXITCODE -eq 0) 'run validator preserves valid schema-v3 backward compatibility'
...
if([version]$version -ge [version]'2.2.0'){
  $pyCmd=Get-Command python -ErrorAction SilentlyContinue
  if($pyCmd){& $pyCmd.Source (Join-Path $root 'scripts\v2.2-dashboard-red-team.py')|Out-Host;Check ($LASTEXITCODE-eq0) 'v2.2 portable Dashboard red-team';& $pyCmd.Source (Join-Path $root 'scripts\v2.2-final-adversarial-red-team.py')|Out-Host;Check ($LASTEXITCODE-eq0) 'v2.2 final portable adversarial red-team';& $pyCmd.Source (Join-Path $root 'scripts\v2.2-dashboard-usability-red-team.py')|Out-Host;Check ($LASTEXITCODE-eq0) 'v2.2 human usability red-team'}else{Check $false 'Python required by v2.2 Dashboard red-team'}
  if($IsWindows -or $env:OS-eq'Windows_NT'){& (Join-Path $root 'scripts\v2.2-windows-final-adversarial-red-team.ps1')|Out-Host;Check ($LASTEXITCODE-eq0) 'v2.2 final Windows adversarial red-team'}
}
